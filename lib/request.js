...
async function handleResponse(response) {
    ...
    if (response.ok) {
        ...
        throw new KlaviyoAuthenticationError(statusCode, responseBody);
    } else if (statusCode == 429) {
        throw new KlaviyoRateLimitError(statusCode, responseBody, Number(response.headers['retry-after']));
    } else if (statusCode == 500 || statusCode == 503) {
        throw new KlaviyoServerError(statusCode, responseBody);
    } else {
        throw new KlaviyoApiError(statusCode, responseBody);
    }
}
